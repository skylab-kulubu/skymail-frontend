...
describe("what a row allows", () => {
  ...
  // Creating a send is the send form's list send (send-form/access.ts): a
  // template to send, sent at once with mails:write and submitted for
  // approval without it (ticket 20). The API sends to an internal list and to
  // a Keycloak group's members alike (handlers/mail.go CreateTask), never to
  // an archived list.
  it("lets someone who can pick a list start a send to a current list or group, sent at once or for approval", () => {
    ...
    assert.equal(listActions(current, [...READER, "skymail:templates:read"]).compose, true);
  });
