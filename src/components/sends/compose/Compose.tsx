...
export function Compose({ access, templates, lists }: { access: SendAccess; templates: MailTemplate[]; lists: ListRow[] }) {
  ...
  const [people, setPeople] = useState<PersonEntry[]>(() => [personEntry()]);
    ...
    <div ref={rootRef} className="space-y-6">
      ...
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-8">
          ...
          {problems ? <NoticeBox tone="error">Gönderim henüz gönderilemez: işaretli yerleri düzelt.</NoticeBox> : null}
          {failure && held ? (
            <NoticeBox tone="warning">
          ...
          <FormActions
            cancel={
              <Button variant="secondary" href={access.detail ? SEND_LIST_PATH : '/'} disabled={busy}>
                İptal
              </Button>
            }
            submit={
              <Button onClick={submit} disabled={busy || held} title={held ? 'Önce yeniden göndermeyi onayla' : undefined}>
